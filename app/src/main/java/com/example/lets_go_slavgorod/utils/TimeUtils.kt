...
object TimeUtils {
    ...
    /**
     * Вычисляет точное время до отправления с учётом секунд
     * 
     * Расширенная версия getTimeUntilDeparture() для отображения обратного отсчёта
     * в реальном времени с точностью до секунды.
     * 
     * Используется в:
     * - CountdownTimer для показа точного времени до отправления
     * - CompactScheduleCard для детального отображения
     * 
     * @param departureTime время отправления в формате "HH:mm"
     * @param currentTime текущее время (по умолчанию системное)
     * @return пара (минуты, секунды) до отправления или null если время прошло
     * 
     * @see getTimeUntilDeparture для получения только минут
     * @see formatTimeUntilDepartureWithSeconds для форматирования результата
     */
