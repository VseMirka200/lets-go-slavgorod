...
/**
 * Центральное хранилище констант приложения "Поехали! Славгород"
 * 
 ...
 * 
 * Архитектура:
 * - Все константы объявлены как объект для глобального доступа
 * - Группировка по функциональным областям
 * - Типизированные значения для безопасности
 * - Документированные значения с описанием назначения
 * 
 * Категории констант:
 * - **Информация о приложении** (версия, название, идентификаторы)
 ...
 * - **База данных** (имя, версия схемы, настройки)
 * - **Цвета** (маршруты, UI элементы, прозрачность)
 * - **Уведомления** (таймауты, каналы, префиксы)
 * - **Удаленная загрузка** (GitHub API, таймауты, URL)
 * - **Поиск и кэширование** (задержки, лимиты, пороги)
 * - **Логирование** (уровни важности, размеры буферов)
 * - **Виджеты** (интервалы обновления, размеры кэша)
 * - **Производительность** (лимиты кэша, пороги очистки)
 * 
 * Особенности:
 * - Thread-safe доступ к константам
 ...
 * - Улучшена производительность инициализации
 * - Обновлены комментарии и документация
 * - Добавлены константы для системы логирования
 * - Обновлены значения для поддержки кириллицы
 * 
 * @author VseMirka200
 * @version 3.0
 * @since 1.0
...
object Constants {
    ...
    /** Версия приложения для отображения пользователю */
    const val APP_VERSION = "v2.1.1"
    
    /** Название приложения */
    const val APP_NAME = "Поехали! Славгород"
    ...
    // ViewModels - настройки кэширования для оптимизации производительности
    const val ROUTES_MAX_CACHE_SIZE = 100                 // Максимальный размер кэша маршрутов
    const val ROUTES_CACHE_CLEANUP_THRESHOLD = 80         // Порог очистки кэша маршрутов (80% от максимума)
    const val SCHEDULE_MAX_CACHE_SIZE = 50                // Максимальный размер кэша расписаний
    
    // Виджеты - настройки обновления для экономии батареи
    const val WIDGET_UPDATE_INTERVAL_MINUTES = 30         // Интервал обновления виджетов (минуты)
    const val WIDGET_CACHE_SIZE = 20                      // Размер кэша для виджетов
    
    // Производительность - лимиты для предотвращения утечек памяти
    const val MAX_FAVORITE_TIMES = 1000                    // Максимальное количество избранных времен
    const val MAX_NOTIFICATION_ALARMS = 50                  // Максимальное количество активных будильников
}