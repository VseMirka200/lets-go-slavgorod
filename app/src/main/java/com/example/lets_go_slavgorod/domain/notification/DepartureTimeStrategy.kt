...
class SelectedDaysStrategy(
    ...
    override fun calculateNextTime(baseTime: Calendar, now: Calendar): Long {
        ...
        if (targetDayOfWeek == null) {
            ...
            // ИСПРАВЛЕНО: Ищем ближайший день, который соответствует дню недели избранного времени
            // И который также входит в выбранные дни
            for (i in 0..14) { // 2 недели
                val candidateDeparture = (baseTime.clone() as Calendar).apply {
                    add(Calendar.DAY_OF_YEAR, i)
                }
                ...
                // Проверяем, что день недели совпадает с целевым И входит в выбранные дни
                val isTargetDay = candidateDayOfWeek == targetDayOfWeek
                val isSelectedDay = candidateDayOfWeek != null && candidateDayOfWeek in selectedDays
                
                if (isTargetDay && isSelectedDay && candidateDeparture.after(now)) {
                    return candidateDeparture.timeInMillis
                }
            }
        }
