...
import com.example.lets_go_slavgorod.ui.components.FeedbackSelection
...
fun AboutScreen(
    ...
    if (showFeedbackDialog) {
        FeedbackTypeDialog(
            ...
            onTypeSelected = { feedbackSelection ->
                ...
                try {
                    ...
                    val subject = "${feedbackSelection.type.emailPrefix} [Поехали! Славгород] $deviceLabel"
                    ...
                    val emailBody = buildString {
                        ...
                        appendLine("Версия: ${Constants.APP_VERSION}")
                        
                        if (feedbackSelection.includeLogs) {
                            appendLine()
                            appendLine("📋 Логи приложения будут прикреплены к письму")
                            appendLine("(Файл будет создан в папке Downloads)")
                        }
                    }
                    ...
                    // Если пользователь выбрал отправку логов, создаем файл логов
                    if (feedbackSelection.includeLogs) {
                        try {
                            // Создаем файл логов в фоновом режиме
                            logsViewModel.exportLogs(context)
                            Timber.d("Создан файл логов для отправки с обратной связью: ${feedbackSelection.type.label}")
                        } catch (e: Exception) {
                            Timber.e(e, "Ошибка создания файла логов для обратной связи")
                        }
                    }
                } catch (e: Exception) {
